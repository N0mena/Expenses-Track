...
class MockApiService {
  ...
  async getExpensesList(filters = {}) {
    ...
    if (filters.start && filters.end) {
      expenses = expenses.filter((exp) => {
        ...
        return (
          (!exp.endDate || exp.endDate >= filters.start) &&
          exp.startDate <= filters.end
        );
      });
    }
  ...
  async getExpense(id) {
    ...
    const expense = expenses.find(
      (exp) => exp.id === id && exp.userId === user.id
    );
  ...
  async getIncomesList(filters = {}) {
    ...
    if (filters.start && filters.end) {
      incomes = incomes.filter(
        (inc) => inc.date >= filters.start && inc.date <= filters.end
      );
    }
  ...
  async updateIncome(id, incomeData) {
    ...
    const index = incomes.findIndex(
      (inc) => inc.id === id && inc.userId === user.id
    );
  ...
  async deleteCategory(id) {
    ...
    const isInUse = expenses.some(
      (exp) => exp.categoryId === id && exp.userId === user.id
    );
  ...
  async getReceipt(expenseId) {
    ...
    const expenses = this.getExpenses();
    const expense = expenses.find(
      (exp) => exp.id === expenseId && exp.userId === user.id
    );
  ...
  async getMonthlySummary(month = null) {
    ...
    const incomes = await this.getIncomesList({
      start: startDate,
      end: endDate,
    });
    const totalIncome = incomes.reduce((sum, inc) => sum + inc.amount, 0);

    const expenses = await this.getExpensesList({
      start: startDate,
      end: endDate,
    });
    let totalExpenses = 0;
    ...
    expenses.forEach((exp) => {
      if (exp.type === "one-time") {
        ...
        if (
          expStartDate <= endOfMonth &&
          (!expEndDate || expEndDate >= startOfMonth)
        ) {
          totalExpenses += exp.amount;
        }
      }
  ...
  async getAlerts() {
    ...
    if (summary.balance < 0) {
      alerts.push({
        ...
        message: `You've exceeded your monthly budget by $${Math.abs(
          summary.balance
        ).toFixed(2)}`,
        severity: "high",
      });
